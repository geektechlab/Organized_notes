...
int main(void)
{
    ...
    printf("**********************************************************\r\n");
    printf("Heap 3 mem allocation example, page 58, 161204.pdf \r\n");
    printf("**********************************************************\r\n");
	...
	if (xTaskCreate(Task1, "task1", TASK_STACK_SIZE, NULL, TASK_PRIORITY, NULL) == pdPASS)
	{
		if (xTaskCreate(Task2, "task2", TASK_STACK_SIZE, NULL, TASK_PRIORITY, NULL) == pdPASS)
		{
			...
			/* Heap_4 is not deterministic, but is faster than most standard library implementations
			 * of malloc() and free(). */

			/* It maintains xStart and xEnd static linked lists within heap_4.c and allocates memory
			...
			/* It maintains xMinimumEverFreeBytesRemaining and xFreeBytesRemaining static variables
			 * in heap_4.c, which tracks amount of minimum free bytes ever remaining and free bytes
			 * remaining at particular time. Similarly, it maintains xNumberOfSuccessfulAllocations
			 * and xNumberOfSuccessfulFrees also keep a track of number of successful allocations
			 * and frees. */
