...
static volatile LED_Vals_t LED_stat_send={0,0,0,"test"};
...
{
	LED_Vals_t *LED_stat_send_ptr = &LED_stat_send;
	volatile uint8_t sendCMD=0;
	while(1)
	{
		if(xQueueSend(ledCmdQueue, &LED_stat_send_ptr, portMAX_DELAY) == pdTRUE)
		{
			sendCMD++;
			sendCMD%=8;
			...
			LED_stat_send.msg[4]=(char)('0'+sendCMD);
			LED_stat_send.msg[5]='\0';
