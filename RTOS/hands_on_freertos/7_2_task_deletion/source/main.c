...
static StackType_t Task2Stack[TASK_STACK_SIZE];
...
int main(void)
{
    ...
    printf("**********************************************************\r\n");
    printf("https://github.com/PacktPublishing/Hands-On-RTOS-with-Microcontrollers/blob/master/Chapter_7/Src/main_taskCreation.c\r\n");
    printf("add breakpoints at different locations and observe execution and variable values\r\n");
    printf("**********************************************************\r\n");
	...
	if (xTaskCreate(GreenTask, "GreenTask", TASK_STACK_SIZE, NULL, TASK_PRIORITY+2, NULL) == pdPASS)
	{
		...
		{
			/* To use static task memory, set configSUPPORT_STATIC_ALLOCATION to 1. */
			if (xTaskCreateStatic(OrangeTask, "OrangeTask", TASK_STACK_SIZE, NULL, TASK_PRIORITY, Task2Stack, &Task2TCB) != NULL)
			{
				/* Can't we delete statically allocated tasks ? can't we delete OrangeTask here ?
