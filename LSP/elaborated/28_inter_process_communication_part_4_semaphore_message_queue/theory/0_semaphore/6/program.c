...
void shutdown_semaphore_mechanism() {
...
int main(int argc, char** argv) {
	...
	// Parent process needs to initialize the semaphore mechanism
	init_semaphore_mechanism();
	...
	void* map = mmap(0, SHARED_SIZE, PROT_WRITE, MAP_SHARED, fd, 0);
	if (map == MAP_FAILED) {
	...
	*counter = 0;
	...
	// the employed semaphore mechanism
	if (pid) {
		shutdown_shared_resource();
		shutdown_semaphore_mechanism();
	}
