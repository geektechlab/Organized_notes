...
int main(void)
{
    ...
    printf("**********************************************************\r\n");
    printf("static task creation with Heap 4 mem allocation example, page 58-61, 161204.pdf \r\n");
    printf("**********************************************************\r\n");
	...
	if (xTaskCreate(Task1, "task1", TASK_STACK_SIZE, NULL, TASK_PRIORITY, NULL) == pdPASS)
	{
		...
		if (xTaskCreateStatic(Task2, "task2", TASK_STACK_SIZE, NULL, TASK_PRIORITY, Task2Stack, &Task2TCB) != NULL)
		{
			/* In heap scheme 4, memory gets allocated by array ucHeap in heap_4.c
			...
			/* In heap scheme 4, memory free is allowed and freed memory will get merged */
			vPortFree(mem);
