...
void sendCMD(void *argument)
{
	while(1)
	{
		...
		vTaskDelay(1000);
		xTaskNotify( recvTaskHandle, RED_LED_MASK, eSetValueWithoutOverwrite);
		vTaskDelay(1000);
	}
}
...
int main(void)
{
    ...
    /* Create the task */
	if (xTaskCreate(recvTask, "Update GREEN LED based on sent cmd", TASK_STACK_SIZE, NULL, TASK_PRIORITY+2, &recvTaskHandle) == pdPASS)
	{
	    CY_ASSERT(recvTaskHandle != NULL);
		if (xTaskCreate(sendCMD, "send LED cmd after each 1000", TASK_STACK_SIZE, NULL, TASK_PRIORITY, NULL) == pdPASS)
		{
