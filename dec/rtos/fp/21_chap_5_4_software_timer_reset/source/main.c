...
int main(void)
{
    ...
    /* Resetting a software timer means to re-start the timer; the timer’s expiry time is
     * recalculated to be relative to when the timer was reset, rather than when the timer was
     * originally started. This is demonstrated by Figure 46, which shows a timer that has a period
     * of 6 being started, then reset twice, before eventually expiring and executing its callback
     * function. */

    /* Figure 46 Starting and resetting a software timer that has a period of 6 ticks */

    /* A timer is reset using the xTimerReset() API function. xTimerReset() can also be used to start
     * a timer that is in the Dormant state. */

    /* Never call xTimerReset() from an interrupt service routine. The interrupt-safe version
     * xTimerResetFromISR() should be used in its place. */

    /* Following can be use case:
     * the behavior of the backlight on a cell phone. The backlight:
     ...
     * - The [simulated] backlight is turned on when a key is pressed, and turned off in the software
