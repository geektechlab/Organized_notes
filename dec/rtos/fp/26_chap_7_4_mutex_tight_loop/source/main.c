...
int main(void)
{
    ...
    /* If a mutex is used in a tight loop by more than one task, and the tasks that use the mutex
     * have the same priority, then care must be taken to ensure the tasks receive an approximately
     * equal amount of processing time. The reason the tasks might not receive an equal amount of
     * processing time is demonstrated by Figure 69, which shows a sequence of execution that could
     * occur if two instances of the task shown by Listing 125 are created at the same priority. */

    /* Understand following listings and related descr: */
     ...
     * processing time, while also ensuring processing time is not wasted by switching between tasks
     * too rapidly */
