...
* Function Prototypes
********************************************************************************/
static volatile uint32_t buf[5];
static uint32_t front=0;
static uint32_t back=0;
SemaphoreHandle_t semPtr_empty = NULL;
SemaphoreHandle_t semPtr_filled = NULL;
 ...
 * which indicates producer it can fill if it wants. Here we are using binary semaphore,
 * because of which we can't use buf more than 1. If we will convert it to counting sempahore,
 * then we will be able to use it more. Check if single counting semaphore will work or we will
 * need 2 counting semaphores.
 */
...
void producerTask(void *argument)
{
	...
	while(1)
	{
		xSemaphoreTake(semPtr_empty, portMAX_DELAY);
...
void displayTask(void *argument)
{
	while(1)
	{
		 ...
		 * from other task.
		 */
		xSemaphoreTake(semPtr_filled, portMAX_DELAY);
...
int main(void)
{
     ...
     * Lock: allow only one section of code to enter into "locked" section of code
     * Mutex: like a lock but system wide and shared by multiple processes. Has priority inheritance
     * 		mechanism to mitigate priority inversion. Can protect single instance of resource only ?
     * Semaphore: Two types: counting and binary.
     * 		Counting: Used to protect multiple instances of resource ? or to latch multiple event counts
     * 		Binary: Used for synchronization. Avoid to protect single instance of resource because it
     * 		is not safe for priority inversion
     */

    mutexPtr_buff_protect = xSemaphoreCreateMutex();
    CY_ASSERT(NULL != mutexPtr_buff_protect);
    semPtr_empty = xSemaphoreCreateBinary();
	CY_ASSERT(semPtr_empty != NULL);
	semPtr_filled = xSemaphoreCreateBinary();
	CY_ASSERT(semPtr_filled != NULL);
